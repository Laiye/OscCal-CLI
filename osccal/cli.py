    ...
    try:
        ...
        for pk in probes:
            if head_id == pk or pk in head_id:
                console.print(f"[green]✓[/green] 自动识别探头: [cyan]{pk}[/cyan] (查询结果: {head_id})")
                return pk
        console.print(f"[dim]⊘[/dim] 探头查询结果 [cyan]{head_id}[/cyan] 未匹配已知型号 {list(probes.keys())}")
    except Exception:
...
def _select_items() -> str:
    ...
    presets = {"0": "all", "1": "amp", "2": "dc_gain", "3": "delta_time", "4": "bandwidth", "5": "transient"}
    if "," in choice:
...
def calibrate(osc, profile, calibrator, channel, probe, items, resource_osc, resource_cal, socket_osc, bandwidth, bd_step, auto):
    """执行示波器校准流程"""
    ...
    if auto:
        ...
        for r in all_resources:
            try:
                ...
                console.print(f"  [green]✓[/green] [cyan]{r}[/cyan] → {idn_info.get('manufacturer','?')} {idn_info.get('model','?')}")
                inst.close()
            except Exception:
        ...
        for r, idn in devices:
            ...
            if cal_device is None and "FLUKE" in mfr:
                ...
                console.print(f"  [dim]⊘[/dim] [cyan]{r}[/cyan] → {idn.get('manufacturer','?')} {idn.get('model','?')} [dim]（忽略）[/dim]")
        ...
        console.print(f"[green]✓[/green] 示波器: [cyan]{osc_device[0]}[/cyan] → {osc_device[1].get('manufacturer','?')} {osc_device[1].get('model','?')}")
        ...
        if not cal_files:
            ...
            return
        cmd_calibrator = load_calibrator(os.path.join(calibrators_dir, cal_files[0]))
        ...
        # 连接校准仪（先连才能查询探头）
        console.print("\n[bold]连接校准仪...[/bold]")
        inst_calibrator, idn_cal = connect_visa(cal_device[0])
        ...
        print_auto_detection_summary(
            osc_device[1], cal_device[1],
            cmd_filepath, profile_filepath, os.path.join(calibrators_dir, cal_files[0]),
            probe,
        )
        ...
        if comm_type == "socket":
            ...
            inst_osc, idn_osc = connect_visa(osc_device[0])
